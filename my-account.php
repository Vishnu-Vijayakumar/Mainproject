...
$sqlq="SELECT login_id from tbl_login where email='$email'";
...
header('location:login.php');
	...
	if(isset($_POST['update']))
	{
		...
		$query=mysqli_query($con,"update userreg set firstname='$name',contactno='$contactno' where logid='$logid'");
		if($query)
		{
...
{
$sql = mysqli_query($con, "SELECT password FROM login WHERE password='{$_POST['cpass']}' && logid='$logid'");
$num=mysqli_fetch_array($sql);
if($num>0)
{
 $con=mysqli_query($con,"update login set password='{$_POST['newpass']}' where logid='$logid'");
echo "<script>
alert('Password Changed Successfully !!');
window.href=location='my-account.php';
</script>";
...
$row = mysqli_fetch_assoc($resu);
$logid= $row['logid'];
$query=mysqli_query($conn,"select * from userreg where logid='$logid'");
